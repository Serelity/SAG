...
冻结的 16 条 development 已完成 v8_dev1 真实 Qwen/V100 运行：5 records、11 rejects、11 repairs、0 truncation，27 次 generation 均 stop。聚合诊断表明主要失败是模型将 issue member 输出为字符串数组，造成大量 `malformed_issue_member` 和 `empty_issue`；不是 JSON、token、显存或 vLLM 问题。v8_dev2 因此保持 output schema/projection/decoder/token 不变，Prompt 增加非空对象格式，validator v2 只对逐字命中四个 clean fields 的字符串候选补 grounding，未命中仍删除。下一步用相同 inference packet、相同 development manifest 和全新 RUN_DIR 运行 `scripts/extract_semantics_v8_dev2.sh`；不得打开 holdout、设置 RESUME 或复用 v8_dev1 目录。仍只回传 safe checker/run/quality/diagnostics summary 和文件 hash，不粘贴正文或 evidence。
