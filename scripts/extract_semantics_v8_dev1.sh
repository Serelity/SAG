...
[[ "${RESUME:-0}" != "1" ]] || {
  echo "v8_dev1 comparisons require a fresh RUN_DIR; RESUME=1 is forbidden" >&2
  exit 2
}
[[ -n "${RUN_DIR:-}" ]] || {
  echo "Set RUN_DIR to a new output directory, e.g. outputs/v8-dev1-smoke-001" >&2
  exit 2
}
[[ ! -e "$RUN_DIR" ]] || {
...
[[ -n "${IDENTITY_MANIFEST:-}" && -f "$IDENTITY_MANIFEST" ]] || {
  echo "Set IDENTITY_MANIFEST to an exact private development/holdout manifest" >&2
  exit 2
}
...
export CONFIG="${CONFIG:-configs/sag_semantic_extraction_qwen3_4b_v8_dev1.json}"
